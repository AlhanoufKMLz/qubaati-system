...
**Folder 14 - Security - Auth & Ownership** (32 requests) proves: 401 (no auth on protected/AI/notification endpoints), 403 (cross-role: student→teacher, parent→teacher, teacher→parent AI, parent/student→create activity, parent/student→evaluate/feedback, teacher→accept recommendation; cross-owner: parent→another parent, teacher→another teacher, user→another user's notifications, parent→non-owned child AI summary, student→non-owned recommendation, classroom create by parent/student, child create by teacher/student), and owner/admin success (own dashboard, own notifications → 200).

### Ownership-by-resource + full `/me` coverage (latest)

- **Generic `POST /activities`** now derives the owner from Basic Auth (`resolveOwningTeacherId`) exactly like classroom create and AI generate — **no `teacherId` in the body**; PARENT/STUDENT → 403.
- **Submission-scoped ownership:** `POST /ai/activity-submissions/{id}/evaluate|generate-feedback`, `PATCH /activity-submissions/{id}/return-to-student|teacher-feedback|reopen`, `GET …/teacher-details`, and `PATCH /student-answers/{id}/grade` now require the current teacher to own the submission's activity/classroom (`createdByTeacher` / classroom `teacher` / `assignedByTeacher`) — admin bypasses; another teacher → 403.
- **Recommendation ownership:** `PATCH /recommendations/{id}/accept|dismiss|complete` are STUDENT-or-ADMIN and require the owning student. New `/students/me/recommendations[/regenerate]`.
- **Complete student `/me` set:** `/students/me`, `/me/skills`, `/me/skills/history`, `/me/learning-style/history`, `/me/recommendations`, `/me/missions/available`, `/me/missions/{id}/regenerate`, `/me/activity-assignments`, `/me/activity-results`, `/me/career-worlds/available`, `/me/activity-dashboard`; mission `start` derives the student (no `studentId` query). Teacher `/me/activity-submissions/pending-grading`; subscription `/subscriptions/{parents,teachers}/me/status`.
- **Lint:** a script scans the normal folders (01–13) for `/parents/{{parentId}}`, `/teachers/{{teacherId}}`, `/students/{{studentId}}`, `/users/{{userId}}` — **0 hits** (child-selection `/parents/me/children/{studentId}` excepted; folder 14 intentionally uses legacy ids to prove protection).
- **Legacy `{id}` routes** are kept for back-compat, **`@Deprecated`**, and ownership-protected.
- **No acting-user id in request bodies:** review (`approve/reject/request-revision`), assignment (`assign/students|classrooms|bulk` + generic `POST /activity-assignments`), `teacher-feedback`, `return-to-student`, and manual `grade` now **derive the teacher from Basic Auth** — the body `teacherId`/`assignedByTeacherId` is optional + `@Deprecated` and ignored for teachers (admin may still supply it). Review is teacher-**owner**-only; assignment requires the teacher to own the activity and (for a student target) that the student is in the teacher's classroom. A Postman lint confirms **0 actor profile ids in normal-folder paths or bodies**.
